...
fn test_with_deny_list_from_files() {
    ...
    .unwrap_or_else(|_| panic!("unable to generate docs"));
    let firs_gen_doc = generated_docs
        .first()
        .map_or_else(|| panic!("unable to find first value"), |val| val.0.clone());
    assert_eq!(firs_gen_doc, PathBuf::from("sql_files/with_mixed_comments.sql"));
    ...
    for (i, (_, sqldoc)) in generated_docs.iter().enumerate() {
        ...
        let sqldoc_tables = sqldoc.tables()[i]
            .doc()
            .as_ref()
            .map_or_else(|| panic!("unable to find sqldoc columns"), |val| val);
        assert_eq!(sqldoc_tables, table_comments[i]);
    }
